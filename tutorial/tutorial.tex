...
To support these primary functions robustly, YODA implements belief tracking and clarification strategies.
...
\subsection{Clarification Dialog}

The initial implementation will support the following types of Clarification dialog acts:

\begin{itemize}[nolistsep]
\item Give Clarification
  \begin{itemize}[nolistsep]
  \item Provide Value
  \item Provide Role
  \item Correct Value
  ...
  \item Reject Role
  \end{itemize}
\item Ask Clarification
  \begin{itemize}[nolistsep]
  \item Non-hearing
  \item Non-understanding
  \item Disambiguate Role
  \item Disambiguate Value
  \item Request Confirm Role
  \item Request Confirm Value
  \end{itemize}
\end{itemize}

The SLU component will often parse clarification acts as fragments.
It is then the role of the dialog state tracker to re-interpret the act as a clarification act.

The designer should set up the slots in their domain to inherit from standard semantic roles.
This will allow the system to automatically generate sensical clarification requests, and to interpret clarification dialog acts correctly.
In our example domain, the Email class has a hasEmailRecipient slot, which inherits from the hasRecipient semantic role.
`Recipient' is a standard semantic role, and YODA can automatically build an SLU system to detect recipients for the SendEmail action.
Furthermore, YODA can automatically engage in clarification dialog related to the hasEmailParticipant slot, based on built-in clarification behavior for the standard Recipient role.

\begin{verbatim}
User: (poor ASR) <NOISE> Sam an email
(SLU result)
confidence = .4
{
 action=send
...
System: Do you want me to send an email to Sam?
 - OR -
System: Do you want me to send Sam an email?
\end{verbatim}

In the above example, the NLG component knows how to generate its clarification request without the designer providing templates because the email and recipient both inherit from standard semantic roles.

There is additional power and complexity due to the distributed nature of slots in YODA.
There is ambiguity as to weather the recipient is a parameter of the send action, or a parameter of the email object.
In the representation we choose here, emails have recipients.
So behind the scenes, the utterance's recipient becomes the theme's EmailRecipient.

The following examples are more complicated.
They demonstrate the unavoidable ambiguity in utterances that describe complex tasks, and show how YODA deals with that ambiguity.


\begin{verbatim}
User: Move the meeting from three to four
{
 action: MoveMeeting
 theme: {class: meeting
         startTime: 3
...
Meeting objects have startTime and endTime slots, which inherit from the fromTime and toTime roles respectively.
